...
class tst_QSparseHash : public QObject
{
    Q_OBJECT
private slots:
    void insert1();
...
typedef QSparseHash<QString, MyClass> MyMap;

//void tst_QMap::count()
void tst_QSparseHash::count()
{
    {
        MyMap map;
        MyMap map2( map );
    ...
    {
        typedef QSparseHash<QString, MyClass> Map;
        Map map;
        map.set_deleted_key(QString());
        QCOMPARE( map.count(), 0);
...
void tst_QSparseHash::insert1()
{
    ...
    {
        typedef QSparseHash<QString, QString> Hash;
        Hash hash;
        hash.set_deleted_key(QString());
        QString key;
        for (int i = 0; i < 10; ++i) {
            key[0] = i + '0';
            for (int j = 0; j < 10; ++j) {
    ...
    {
        typedef QSparseHash<int, const char *> Hash;
        Hash hash;
        ...
        {
            QSparseHash<int,Foo> hash;
            hash.set_deleted_key(-1);
            for (int i = 0; i < 10; i++)
                hash.insert(i, Foo());
            QVERIFY(Foo::count == 10);
            hash.remove(7);
            QVERIFY(Foo::count == 9 + 1); // Elements are not really removed - default constructed Foo will be used

        }
        QVERIFY(Foo::count == 0);
        {
            QSparseHash<int, int*> hash;
            QVERIFY(((const QSparseHash<int,int*>*) &hash)->operator[](7) == 0);
        }
    }
}

void tst_QSparseHash::erase()
{
    ...
    h1.insert(5, 6);
    QVERIFY(h1.size() == 2);
    QSparseHash<int, int>::iterator it1 = h1.begin();
    ++it1;
    it1 = h1.erase(it1);
    QVERIFY(it1 == h1.end());
    h1.insert(7, 8);
...
void tst_QSparseHash::key()
{
    {
        QString def("default value");

        QSparseHash<QString, int> hash1;
...

    {
        ...
        QSparseHash<int, QString> hash2;
        hash2.set_deleted_key(-100);
...
void tst_QSparseHash::swap()
{
    QSparseHash<int,QString> h1, h2;
    h1.set_deleted_key(-1);
    h1[0] = "h1[0]";
    h2[1] = "h2[1]";
    h1.swap(h2);
...
//copied from tst_QMap
void tst_QSparseHash::empty()
{
    QSparseHash<int, QString> map1;
...
//    QSparseHash<int, QString>::const_iterator it=map1.constFind(4);
...
// copied from tst_QMap
void tst_QSparseHash::constFind()
{
    QSparseHash<int, QString> map1;
//    QString testString="Teststring %0";
//    QString compareString;
...
// copied from tst_QMap
void tst_QSparseHash::contains()
{
    QSparseHash<int, QString> map1;
    map1.set_deleted_key(-111);
    int i;
...
//copied from tst_QMap
void tst_QSparseHash::take()
{
    QSparseHash<int, QString> map;
...
//copied from tst_QMap
void tst_QSparseHash::operator_eq()
{
    {
        // compare for equality:
        QSparseHash<int, int> a;
        QSparseHash<int, int> b;
        a.set_deleted_key(-10);
        b.set_deleted_key(-10);
    ...
    {
        // a more complex map
        QSparseHash<QString, QString> a;
        QSparseHash<QString, QString> b;
        a.set_deleted_key(QString("xxx"));
        b.set_deleted_key(QString("xxx"));
    ...
    {
        QSparseHash<QString, int> a;
        QSparseHash<QString, int> b;
...
void tst_QSparseHash::compare()
{
    QSparseHash<int, QString> hash1,hash2;
...
void tst_QSparseHash::iterators()
{
    QSparseHash<int, QString> hash;
    ...
    QSparseHash<int, QString>::iterator stlIt = hash.begin();
    ...
    QSparseHash<int, QString>::const_iterator cstlIt = hash.constBegin();
    ...
    QSparseHashIterator<int, QString> javaIt(hash);
...
void tst_QSparseHash::rehash_isnt_quadratic()
{
//    // this test should be incredibly slow if rehash() is quadratic
//    for (int j = 0; j < 5; ++j) {
//        QSparseHash<int, int> testHash;
...
void tst_QSparseHash::dont_need_default_constructor()
{
    QSparseHash<int, Bar> hash1;
    for (int i = 0; i < 100; ++i) {
...
void tst_QSparseHash::qmultihash_specific()
{
...
void tst_QSparseHash::keys_values_uniqueKeys()
{
...
void tst_QSparseHash::noNeedlessRehashes()
{
//    QSparseHash<int, int> hash;
//    for (int i = 0; i < 512; ++i) {
//        int j = (i * 345) % 512;
...
void tst_QSparseHash::const_shared_null()
{
//    QSparseHash<int, QString> hash1;
...
struct OneArgumentQSparseHashStruct1 {};
bool operator==(const OneArgumentQSparseHashStruct1 &, const OneArgumentQSparseHashStruct1 &) { return false; }
uint qHash(OneArgumentQSparseHashStruct1) { return 0; }

struct OneArgumentQSparseHashStruct2 {};
bool operator==(const OneArgumentQSparseHashStruct2 &, const OneArgumentQSparseHashStruct2 &) { return false; }
uint qHash(const OneArgumentQSparseHashStruct2 &) { return 0; }

struct OneArgumentQSparseHashStruct3 {};
bool operator==(const OneArgumentQSparseHashStruct3 &, const OneArgumentQSparseHashStruct3 &) { return false; }
uint qHash(OneArgumentQSparseHashStruct3) { return 0; }
uint qHash(OneArgumentQSparseHashStruct3 &, uint) { wrongqHashOverload = 1; return 0; }

struct OneArgumentQSparseHashStruct4 {};
bool operator==(const OneArgumentQSparseHashStruct4 &, const OneArgumentQSparseHashStruct4 &) { return false; }
uint qHash(const OneArgumentQSparseHashStruct4 &) { return 0; }
uint qHash(OneArgumentQSparseHashStruct4 &, uint) { wrongqHashOverload = 1; return 0; }


struct TwoArgumentsQSparseHashStruct1 {};
bool operator==(const TwoArgumentsQSparseHashStruct1 &, const TwoArgumentsQSparseHashStruct1 &) { return false; }
uint qHash(const TwoArgumentsQSparseHashStruct1 &) { wrongqHashOverload = 1; return 0; }
uint qHash(const TwoArgumentsQSparseHashStruct1 &, uint) { return 0; }

struct TwoArgumentsQSparseHashStruct2 {};
bool operator==(const TwoArgumentsQSparseHashStruct2 &, const TwoArgumentsQSparseHashStruct2 &) { return false; }
uint qHash(TwoArgumentsQSparseHashStruct2) { wrongqHashOverload = 1; return 0; }
uint qHash(const TwoArgumentsQSparseHashStruct2 &, uint) { return 0; }

struct TwoArgumentsQSparseHashStruct3 {};
bool operator==(const TwoArgumentsQSparseHashStruct3 &, const TwoArgumentsQSparseHashStruct3 &) { return false; }
uint qHash(const TwoArgumentsQSparseHashStruct3 &) { wrongqHashOverload = 1; return 0; }
uint qHash(TwoArgumentsQSparseHashStruct3, uint) { return 0; }

struct TwoArgumentsQSparseHashStruct4 {};
bool operator==(const TwoArgumentsQSparseHashStruct4 &, const TwoArgumentsQSparseHashStruct4 &) { return false; }
uint qHash(TwoArgumentsQSparseHashStruct4) { wrongqHashOverload = 1; return 0; }
uint qHash(TwoArgumentsQSparseHashStruct4, uint) { return 0; }

/*!
    \internal

    Check that QSparseHash picks up the right overload.
...
*/
void tst_QSparseHash::twoArguments_qHash()
{
    QSparseHash<OneArgumentQSparseHashStruct1, int> oneArgHash1;
    OneArgumentQSparseHashStruct1 oneArgObject1;
    oneArgHash1[oneArgObject1] = 1;
    QCOMPARE(wrongqHashOverload, 0);

    QSparseHash<OneArgumentQSparseHashStruct2, int> oneArgHash2;
    OneArgumentQSparseHashStruct2 oneArgObject2;
    oneArgHash2[oneArgObject2] = 1;
    QCOMPARE(wrongqHashOverload, 0);

    QSparseHash<OneArgumentQSparseHashStruct3, int> oneArgHash3;
    OneArgumentQSparseHashStruct3 oneArgObject3;
    oneArgHash3[oneArgObject3] = 1;
    QCOMPARE(wrongqHashOverload, 0);

    QSparseHash<OneArgumentQSparseHashStruct4, int> oneArgHash4;
    OneArgumentQSparseHashStruct4 oneArgObject4;
...
//    QSparseHash<TwoArgumentsQSparseHashStruct1, int> twoArgsHash1;
//    TwoArgumentsQSparseHashStruct1 twoArgsObject1;
//    twoArgsHash1[twoArgsObject1] = 1;
//    QCOMPARE(wrongqHashOverload, 0);

//    QSparseHash<TwoArgumentsQSparseHashStruct2, int> twoArgsHash2;
//    TwoArgumentsQSparseHashStruct2 twoArgsObject2;
//    twoArgsHash2[twoArgsObject2] = 1;
//    QCOMPARE(wrongqHashOverload, 0);

//    QSparseHash<TwoArgumentsQSparseHashStruct3, int> twoArgsHash3;
//    TwoArgumentsQSparseHashStruct3 twoArgsObject3;
//    twoArgsHash3[twoArgsObject3] = 1;
//    QCOMPARE(wrongqHashOverload, 0);

//    QSparseHash<TwoArgumentsQSparseHashStruct4, int> twoArgsHash4;
